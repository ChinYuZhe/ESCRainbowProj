...
let options = {
    ...
    credentials: {
        login: "yuzhe_chin@mymail.sutd.edu.sg", // To replace by your developer credendials
        password: "Zxcvbnm12345." // To replace by your developer credentials
    },
...
rainbowSDK.events.on("rainbow_onready", async () => {
		...
		let userId = "5e9d26399cff6b7279a8daf9";
		let objData = {"agentJobTitles" : "replacement of parts", "agentTitles" : "Epad", "isActive" : "true", "nickName" : "['cs1', 'cs2']"};
		await rainbowSDK.admin.updateInformationForUser(objData,agentID).then((user) => {
			console.log("Done!");
			}).catch((err) => {
			console.log(err);
		...
		let userId = "5e9d25ba9cff6b7279a8dade";
		let objData = {"agentJobTitles" : "hardware", "agentTitles" : "Ephone", "isActive" : "false", "nickName" : "[]"};
		await rainbowSDK.admin.updateInformationForUser(objData,agentID).then((user) => {
			console.log("Done!");
			}).catch((err) => {
			console.log(err);
		...
		let userId = "5e9d264d9cff6b7279a8db0b";
		let objData = {"agentJobTitles" : "maintainance of parts", "agentTitles" : "Emac", "isActive" : "true", "nickName" : "[]"};
		await rainbowSDK.admin.updateInformationForUser(objData,agentID).then((user) => {
			console.log("Done!");
			}).catch((err) => {
			console.log(err);
